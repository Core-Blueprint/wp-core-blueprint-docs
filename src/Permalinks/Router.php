...
final class Router {
	...
	public static function register_rules(): void {
		...
		// Preserve Simple-mode documents whose slug equals a reserved root.
		add_rewrite_rule(
			'^' . $base . '/(tag|document)/?$',
			'index.php?' . self::QUERY_ROUTE . '=legacy-simple&' . self::QUERY_PATH . '=$matches[1]',
			'top'
		);
	...
	public static function redirect_legacy(): void {
		...
		if ( 'legacy-category' === $route ) {
			...
			self::redirect_to( $term_id > 0 ? CanonicalPath::category_url( $term_id ) : null );
			return;
		}
		...
		if ( 'legacy-tag' === $route ) {
			...
			self::redirect_to(
				$term instanceof \WP_Term
					? CanonicalPath::tag_url( (string) $term->slug )
					: null
			);
			return;
		}
		...
		if ( 'legacy-simple' === $route && is_singular( PostType::TYPE ) ) {
			self::redirect_to( CanonicalPath::document_url( (int) get_queried_object_id() ) );
			return;
		}
	...
	private static function redirect_to( ?string $url ): void {
		...
		if (
			'' !== $current_path
			&& untrailingslashit( $current_path ) === untrailingslashit( $target_path )
		) {
			return;
		}
